package minecraft.phoenix.scienceExp.gases;
 ...
 *
 **/
public class BlockRenderGas implements ISimpleBlockRenderingHandler
{
    ...
    public BlockRenderGas(int renderId)
    {
    ...
    public float getFluidHeightForRender(IBlockAccess world, int x, int y, int z, BlockFluidGas block)
    {
        ...
        return !world.getBlockMaterial(x, y, z).isSolid() && world.getBlockId(x, y - block.getDensityDir(), z) == block.blockID ? 1 : block.getQuantaPercentage(world, x, y, z) * 0.875F;
    }
    ...
    {
        if (!(block instanceof BlockFluidGas))
        {
            return false;
        }
        ...
        BlockFluidGas theFluid = (BlockFluidGas) block;
        int bMeta = world.getBlockMetadata(x, y, z);

        boolean renderTop = world.getBlockId(x, y - theFluid.getDensityDir(), z) != theFluid.blockID;

        boolean renderBottom = block.shouldSideBeRendered(world, x, y + theFluid.getDensityDir(), z, 0) && world.getBlockId(x, y + theFluid.getDensityDir(), z) != theFluid.blockID;
        ...
        else
        {
            ...
            if (renderer.renderAllFaces || renderTop)
            {
                ...
                float flowDir = (float) BlockFluidGas.getFlowDirection(world, x, y, z);
